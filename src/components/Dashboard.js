...
export const Dashboard = ({ me }) => {
  ...
  const [dataStoreSupervisionsConfigs, setDataStoreSupervisionsConfigs] =
    useState([]);
  ...
  const [selectedOrganisationUnit, setSelectedOrganisationUnit] =
    useState(null);
  const [selectedPlanification, setSelectedPlanification] =
    useState(MES_PLANIFICATIONS);
  const [selectedPeriod, setSelectedPeriod] = useState(dayjs(new Date()));
  const [selectedPlanificationUser, setSelectedPlanificationUser] =
    useState(null);
  ...
  const [
    loadingDataStoreSupervisionPlanifications,
    setLoadingDataStoreSupervisionPlanifications,
  ] = useState(false);
  const [
    loadingDataStoreSupervisionsConfigs,
    setLoadingDataStoreSupervisionsConfigs,
  ] = useState(false);
  ...
  const loadOrganisationUnits = async () => {
    try {
      ...
      if (progs.length > 0) {
        ...
        const currentOrgUnit = orgUnits.find(
  ...
  const handleSelectSupervisor = (values) =>
    setSelectedSupervisors(
      values.map((sup) => users.find((u) => u.id === sup))
    );

  const handleSelectPlanificationUser = (value) =>
    setSelectedPlanificationUser(users.find((u) => u.id === value));
              ...
              ...prev,
              {
                ...
                agent: `${
                  current.attributes?.find(
                    (att) =>
                      att.attribute === selectedProgram?.attributeName?.id
                  )?.value || ""
                } ${
                  current.attributes?.find(
                    (att) =>
      ...
      .filter((planification) => {
        ...
        if (selectedPlanification === PLANIFICATION_PAR_MOI)
          return (
            ...
            planification.storedBy?.toLowerCase()
          );
  ...
  const getPieChartDatasForSupervisions = () => ({
    ...
    color: [
      { code: SCHEDULED.value, id: null, displayName: SCHEDULED.name },
    ...
    series: [
      {
        ...
        data: [
          ...
          return (
            statusPayload[option.code] || { name: option.displayName, value: 0 }
          );
        }),
        emphasis: {
          itemStyle: {
  ...
  const getPieChartDatasForPayment = () => ({
    ...
    color: statusPaymentOptions.map(
      (option) => getStatusNameAndColorForPayment(option.code).color.background
    ),
            ...
            (prev, curr) => {
              if (curr.statusPayment && prev[`${curr.statusPayment}`]) {
                prev[`${curr.statusPayment}`] = {
                  name: getStatusNameAndColorForPayment(curr.statusPayment)
                    ?.name,
      ...
      title: (
        <div
          style={{
            ...
            fontSize: "12px",
            borderRadius: "5px",
            backgroundColor: getStatusNameAndColor(
              planification.statusSupervision
            )?.color?.background,
                      ...
                      >
                        <div
                          style={{
                            ...
                            padding: "4px",
                            color: `${
                              getStatusNameAndColor(option.code)?.color?.text
                            }`,
                          }}
                        >
                          {option.displayName}
                  ...
                  <Col flex="auto">
                    <div
                      ...
                      style={{
                        ...
                        height: "100%",
                        borderLeft: `3px solid ${
                          getStatusNameAndColorForPayment(option.code)?.color
                            ?.background
                        }`,
                      }}
                    >
                      <div>
                        <div
                          style={{
                            fontWeight: "bold",
                            color: `${
                              getStatusNameAndColorForPayment(option.code)
                                ?.color?.background
                            }`,
                            textAlign: "center",
                          }}
  ...
  const handleSelectProgram = (value) => {
    if (value) {
      const supFound = dataStoreSupervisionsConfigs.find(
    ...
    <>
      ...
      >
        <Row gutter={[8, 8]} align="middle">
          ...
          {0 > 1 &&
            selectedPlanification === PLANIFICATION_PAR_UN_USER &&
            users.length > 0 && (
              <Col sm={24} md={4}>
                <div style={{ marginBottom: "2px" }}>
