                    ...
                    <Col sm={24} md={3}>
                        ...
                        <Select
                            ...
                            options={organisationUnitLevels
                                .filter(l =>
                                    selectedOrganisationUnit?.level ? l.level >= selectedOrganisationUnit?.level : true
                                )
                                .map(d => ({
                                    value: d.id,
                                    label: d.displayName
            ...
            <div key={uuid()} style={{ marginBottom: '40px' }}>
                ...
                <Row gutter={[8, 8]}>
                    {dataStoreVisualizations
                        .find(vis => selectedProgram?.program?.id && vis.program?.id === selectedProgram?.program?.id)
                        ?.visualizations?.map(v => (
                            <VisualizationItem key={uuid()} id={`${v.id}-${ou?.id}-${ou?.event?.event}`} />
                        ))}
                </Row>
            </div>
        ));
            ...
            concerningOUs.length === +currentPosition + 1 && (
                <div style={{ marginTop: '20px' }}>
                    <div key={uuid()} style={{ marginBottom: '40px' }}>
                        ...
                        >
                            <span style={{ fontSize: '15px' }}>{`${translate('Global_Analysis_For')}  - `}</span>
                            ...
                            {concerningOUs.map(ou => (
                                ...
                                >
                                    {`${ou.displayName} ( ${dayjs(ou.event?.eventDate).format('YYYY-MM-DD')} )`}
                                </span>
                            ))}
                        </div>
                        <Row gutter={[8, 8]}>
                            {dataStoreVisualizations
                                .find(
                                    vis =>
                                        selectedProgram?.program?.id && vis.program?.id === selectedProgram?.program?.id
                                )
                                ?.visualizations?.map(v => (
                    ...
                    for (let item of listItems) {
                        ...
                        if (tspan) {
                            ...
                            if (
                                ...
                                const indicatorName =
                                    currentIndicator &&
                                    output.event?.dataValues?.find(dv => dv.dataElement === currentIndicator?.value?.id)
                                        ?.value;
                            ...
                            if (
                                ...
                                const primaryCrosscheckName =
                                    ...
                                    output.event?.dataValues?.find(
                                        dv => dv.dataElement === currentRecoupement?.primaryValue?.id
                                    )?.value;

                                const secondaryCrosscheckName =
                                    currentRecoupement &&
                                    output.event?.dataValues?.find(
                                        dv => dv.dataElement === currentRecoupement?.secondaryValue?.id
                                    )?.value;
                                    ...
                                    if (ind.keyWords?.length > 0) {
                                        for (let word of ind.keyWords) {
                                            if (text?.trim()?.includes(word.trim())) {
                                                ...
                                                if (indicatorName) {
                                                    tspan.innerHTML = text.replaceAll(word, indicatorName);
                                                }
                                            }
                                        }
                                    }
                                                        ...
                                                        if (legendHighChartParent) {
                                                            ...
                                                            <span style="background : ${
                                                             ...
                                                             <span> ${text.replaceAll(
                                                                 word,
                                                                 primaryCrosscheckName + ' / ' + secondaryCrosscheckName
                                                             )}</span>
                                                            </div>`;
                                                            foundElementLegend.appendChild(legendContent);
                                                        }
            ...
            concerningOUs.slice(currentPosition, currentPosition + +numberOfGeneration).forEach(output => {
                ...
                dataStoreVisualizations
                    ...
                    ?.visualizations?.forEach(v => {
                        ...
                        const rightElement = document.getElementById(`${v.id}-${output?.id}-${output?.event?.event}`);
