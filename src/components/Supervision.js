...
import {
  getDefaultStatusPaymentIfStatusIsNull,
  getDefaultStatusSupervisionIfStatusIsNull,
} from "./Dashboard";
...
const Supervision = ({ me }) => {
  const [dataStoreSupervisionConfigs, setDataStoreSupervisionConfigs] =
    useState([]);
  ...
  const [
    ...
    setFavoritBackgroundInformationList,
  ] = useState([]);

  const [visibleTeamLeadContent, setVisibleTeamLeadContent] = useState(false);
  const [visibleAnalyticComponentModal, setVisibleAnalyticComponentModal] =
    useState(false);
  const [
  ...
  const [
    ...
    setVisibleAddFavoritPerformanceModal,
  ] = useState(false);
  const [
    visibleAddFavoritBackgroundInformationModal,
    setVisibleAddFavoritBackgroundInformationModal,
  ] = useState(false);

  const [
    selectedBackgroundInformationTypeConfiguration,
    setSelectedBackgroundInformationTypeConfiguration,
  ] = useState(DIRECTE);
  const [
  ...
  const [selectedPlanificationType, setSelectedPlanificationType] =
    useState(null);
  ...
  const [
    ...
    setSelectedOrganisationUnitGroupSet,
  ] = useState(null);
  ...
  const [selectedOrganisationUnitSingle, setSelectedOrganisationUnitSingle] =
    useState(null);
  const [selectedAgents, setSelectedAgents] = useState([]);
  const [
    selectedSupervisionsConfigProgram,
    setSelectedSupervisionConfigProgram,
  ] = useState(null);
  const [
    selectedOrgUnitSupervisionFromTracker,
    setSelectedOrgUnitSupervisionFromTracker,
  ] = useState(null);
  const [selectedPeriodSupervisionConfig, setSelectedPeriodSupervisionConfig] =
    useState(null);
  const [selectedOrganisationUnitGroups, setSelectedOrganisationUnitGroups] =
    useState([]);
  const [selectedEquipeSuperviseurs, setSelectedEquipeSuperviseurs] = useState(
    []
  );
  const [selectedEquipeAutreSuperviseurs, setSelectedEquipeAutreSuperviseurs] =
    useState([]);
  const [
    selectedSelectionTypeForPerformance,
    setSelectedSelectionTypeForPerformance,
  ] = useState(DIRECTE);
  const [selectedElementForPerformances, setSelectedElementForPerformances] =
    useState([]);
  const [selectedFavoritForPerformance, setSelectedFavoritForPerformance] =
    useState(null);
  const [selectedDataElementFromWhere, setSelectedDataElementFromWhere] =
    useState(ELEMENT_GROUP);
  const [selectedDataElementGroup, setSelectedDataElementGroup] =
    useState(null);
  ...
  const [inputEquipeAutreSuperviseur, setInputEquipeAutreSuperviseur] =
    useState("");
  ...
  const [inputDataSourceDisplayName, setInputDataSourceDisplayName] =
    useState("");
  ...
  const [
    ...
    setLoadingDataStoreSupervisionConfigs,
  ] = useState(false);
  const [
    loadingSaveFavoritBackgroundInformations,
    setLoadingSaveFavoritBackgroundInformations,
  ] = useState(false);
  const [loadingDataStoreSupervisions, setLoadingDataStoreSupervisions] =
    useState(false);
  const [
    loadingDataStoreIndicatorConfigs,
    setLoadingDataStoreIndicatorConfigs,
  ] = useState(false);
  const [loadingOrganisationUnits, setLoadingOrganisationUnits] =
    useState(false);
  const [
    loadingOrganisationUnitGroupSets,
    setLoadingOrganisationUnitGroupSets,
  ] = useState(false);
  const [loadingUsers, setLoadingUsers] = useState(false);
  const [loadingProgramStages, setLoadingProgramStages] = useState(false);
  const [
    loadingSaveDateElementMappingConfig,
    setLoadingSaveDateElementMappingConfig,
  ] = useState(false);
  const [loadingSupervisionPlanification, setLoadingSupervisionPlanification] =
    useState(false);
  const [loadingAnalyticIndicatorResults, setLoadingAnalyticIndicatorResults] =
    useState(false);
  const [loadingTeiList, setLoadingTeiList] = useState(false);
  const [
    loadingAllSupervisionsFromTracker,
    setLoadingAllSupervisionsFromTracker,
  ] = useState(false);
  const [
    loadingOrgUnitsSupervisionsFromTracker,
    setLoadingOrgUnitsSupervisionsFromTracker,
  ] = useState(false);
  const [loadingOrganisationUnitGroups, setLoadingOrganisationUnitGroups] =
    useState(false);
  const [
    loadingPerformanceFavoritsConfigs,
    setLoadingPerformanceFavoritsConfigs,
  ] = useState(false);
  const [
    loadingBackgroundInformationFavoritsConfigs,
    setLoadingBackgroundInformationFavoritsConfigs,
  ] = useState(false);
  const [loadingDataElementGroups, setLoadingDataElementGroups] =
    useState(false);
    ...
    try {
      const newDataValues = rowEvent.dataValues
        ...
        ? rowEvent.dataValues.map((dvEl) => {
            if (
              selectedSupervisionsConfigProgram?.statusSupervision?.dataElement
                ?.id === dvEl
            ) {
              return {
                ...dvEl,
  ...
  const columns = () => {
    return selectedSupervisionsConfigProgram?.planificationType ===
      ORGANISATION_UNIT
      ? [
          {
                  ...
                  <span
                    ...
                    style={{
                      ...
                      background: getStatusNameAndColor(cell.getValue())?.color
                        ?.background,
                      color: getStatusNameAndColor(cell.getValue())?.color
                        ?.text,
                    }}
                  >
  ...
  const loadOrganisationUnits = async () => {
    try {
      ...
      const userOrganisationUnitId =
        me.organisationUnits?.length > 0 && me.organisationUnits?.[0].id;
  ...
  const loadDataElementGroups = async () => {
    try {
      ...
      const response = await axios.get(
        `${DATA_ELEMENT_GROUPS_ROUTE}?paging=false&fields=id,displayName`
      );
  ...
  const loadTeisPlanifications = async (
    program_id,
    orgUnit_id,
    ouMode = DESCENDANTS
  ) => {
  ...
  const loadAllSupervisionsFromTracker = async (
    dataStoreprogramList,
    orgUnitList
  ) => {
    try {
      setLoadingAllSupervisionsFromTracker(true);
      if (dataStoreprogramList.length > 0) {
        ...
        const currentOrgUnit = orgUnitList.find(
  ...
  const loadDataStorePerformanceFavoritsConfigs = async () => {
    try {
      ...
      const response = await loadDataStore(
        process.env.REACT_APP_PERFORMANCE_FAVORITS_KEY,
        null,
        null,
        []
      );
  ...
  const loadDataStoreSupervisions = async () => {
    try {
      ...
      const response = await loadDataStore(
        process.env.REACT_APP_SUPERVISIONS_KEY,
        null,
        null,
        null
      );
  ...
  const handleSelectedDataElementGroup = (value) => {
    setSelectedDataElementGroup(
      dataElementGroups.find((dxGroup) => dxGroup.id === value)
    );
    setSelectedDataElement(null);
  };
  ...
  const getTeamLead = (dataStoreList = [], eventId) =>
    dataStoreList.reduce((prev, curr) => {
      const found_sup = curr.supervisions?.find(
        (sup) => eventId === sup.tei_event?.event
      );
      if (found_sup) {
  ...
  const filterAndGetPlanfications = () =>
    allSupervisionsFromTracker
      .reduce((prev, current) => {
        if (
          selectedSupervisionsConfigProgram.generationType ===
          TYPE_GENERATION_AS_TEI
        ) {
          if (
            ...
            current.enrollments?.filter(
              (en) =>
                en.program === selectedSupervisionsConfigProgram?.program?.id
            )
          ) {
            if (
              current.enrollments?.filter(
              ...
              ...prev,
              {
                ...
                agent: `${
                  ...
                  current.attributes?.find(
                    (att) =>
                      att.attribute ===
                      selectedSupervisionsConfigProgram?.attributeFirstName?.id
                  )?.value || ""
                }`,
      ...
      .sort(
        (a, b) =>
          parseInt(dayjs(b.period).valueOf()) -
          parseInt(dayjs(a.period).valueOf())
      );
  ...
  const handleSaveNewMappingConfig = async () => {
    try {
      ...
      if (
        ...
        inputDataSourceDisplayName?.trim().length === 0
      )
  ...
  const handleDeleteConfigItem = async (value) => {
    try {
      if (value) {
        const newList = mappingConfigs.filter(
          (mapConf) => mapConf.id !== value.id
        );
        setMappingConfigs(newList);
        setNotification({
          show: true,
  ...
  const handleClickSupervisionItem = (sup) => {
    ...
    if (
      ...
      sup?.attributesToDisplay?.length === 0
    ) {
      return setNotification({
        show: true,
  ...
  const generateTeiWithEnrollmentWithEvents = async (payload) => {
    try {
      ...
      const generatedCode = await generatedAutoCode(
        currentProgram.data.programTrackedEntityAttributes[0]
      ...
      const tei = {
        ...
        attributes: [
          { attribute: generatedCode.ownerUid, value: generatedCode.value },
        ],
      };
      ...
      const enrollment_id =
        createdEnrollment?.response?.importSummaries[0]?.reference;
      ...
      for (let stage of availableProgramStages) {
        ...
        if (
          payload.fieldConfig?.supervisor?.programStage?.id === stage &&
          payload.fieldConfig?.supervisor?.dataElements?.length > 0
        ) {
          const newDataValues = [];
          ...
          if (
            payload.fieldConfig?.supervisor?.dataElements?.length <
            ...
            for (
              let i = 0;
              i < payload.fieldConfig?.supervisor?.dataElements?.length;
              i++
            ) {
              for (let j = 0; j < supervisorArrayCurrent.length; j++) {
                if (i === j) {
                  const currentDE =
                    payload.fieldConfig?.supervisor?.dataElements[i];
                  ...
                  if (
                    ...
                    if (
                      i ===
                      payload.fieldConfig?.supervisor?.dataElements?.length - 1
                    ) {
                      newDataValues.push({
  ...
  const generateEventsAsNewSupervision = async (payload) => {
    try {
      ...
      const existingTEI_List =
        existingTEI_List_response.data.trackedEntityInstances;
  ...
  const handleSelectIndicators = (values) =>
    setSelectedIndicators(
      values.map((val) =>
        dataStoreIndicatorConfigs.find((dsInd) => dsInd.indicator?.id === val)
      )
    );
  ...
  const handleSelectOrgUnitGroup = (values) =>
    setSelectedOrganisationUnitGroups(
      values.map((val) =>
        organisationUnitGroups.find((orgGp) => orgGp.id === val)
      )
    );
  ...
  const saveSupervisionAsTEIStrategy = async (
    inputFieldsList,
    ...
    try {
      if (inputFieldsList.length > 0) {
        ...
        for (let item of inputFieldsList) {
          ...
          const createdTEIObject = await generateTeiWithEnrollmentWithEvents(
            payload
          );
  ...
  const saveSupervisionAsEventStrategy = async (
    inputFieldsList,
    newDataStoreSupervisions
  ) => {
    try {
    ...
    try {
      ...
      for (let stage of availableProgramStages) {
        const eventPayload = {
          ...
          programStage: stage,
          trackedEntityInstance:
            existing_tei.enrollments[0].trackedEntityInstance,
          dataValues: [],
        };
      ...
      const currentTEI = await axios.get(
        `${TRACKED_ENTITY_INSTANCES_ROUTE}/${existing_tei.enrollments[0].trackedEntityInstance}?fields=*,enrollments`
      );
      return currentTEI.data;
    } catch (err) {
      throw err;
  ...
  const saveSupervisionAsEnrollmentStrategy = async (
    inputFieldsList,
    newDataStoreSupervisions
  ) => {
    try {
      if (inputFieldsList.length > 0) {
        ...
        for (let item of inputFieldsList) {
          ...
          const createdTEIObject = await generateEnrollmentsAsNewSupervision(
            payload
          );
          if (createdTEIObject) {
            supervisionsList.push({
  ...
  const handleSupervisionPlanificationSaveBtn = async () => {
    try {
      ...
      if (
        ...
        await saveSupervisionAsTEIStrategy(
          inputFields,
          newDataStoreSupervisions
        );
      ...
      if (
        ...
        await saveSupervisionAsEnrollmentStrategy(
          inputFields,
          newDataStoreSupervisions
        );
      ...
      if (
        ...
        await saveSupervisionAsEventStrategy(
          inputFields,
          newDataStoreSupervisions
        );
  ...
  const handleSelectSupervisionProgramConfigForTracker = (value) => {
    if (value) {
      const supFound = dataStoreSupervisionConfigs.find(
        (d) => d.program?.id === value
      );
  ...
  const calculateMontant = (
    ...
    montant = parseFloat(
      correctPaymentObject.montantConstant -
        (correctPaymentObject.montantConstant *
          correctPaymentObject.fraisMobileMoney) /
          100
    ).toFixed(2);
  ...
  const RenderFloatingButton = () => (
    <>
      <FloatButton
        tooltip={
          isEditionMode
            ? translate("List_Supervisions")
            : translate("Nouvelle_Supervision")
        }
        icon={
          isEditionMode ? (
  ...
  const handleAddNewEquipeSave = () => {
    try {
      if (
        ...
        selectedEquipeSuperviseurs.length === 0
      )
        throw new Error(
          translate("Please_Select_One_Supervisor_Or_Other_Supervisor")
        );
  ...
  const handleSelectSuperviseurs = (values) =>
    setSelectedEquipeSuperviseurs(
      values.map((value) => users.find((u) => u.id === value))
    );

  const handleAddEquipeAutreSuperviseurs = () => {
    if (
      inputEquipeAutreSuperviseur &&
      !selectedEquipeAutreSuperviseurs?.includes(
                    ...
                    <Row gutter={[10, 10]}>
                      <Col md={19} sm={24}>
                        <Input
                          ...
                          style={{ width: "100%" }}
                          onChange={(event) =>
                            setInputEquipeAutreSuperviseur(event.target.value)
                          }
                        />
                      </Col>
                        ...
                        <Button
                          ...
                          disabled={
                            ...
                            selectedEquipeAutreSuperviseurs
                              ...
                              .includes(inputEquipeAutreSuperviseur?.trim())
                          }
                        >
                          + {translate("Ajouter")}{" "}
                        </Button>
  ...
  const RenderAddFavoritBackgroundInformationModal = () =>
    visibleAddFavoritBackgroundInformationModal && (
      <>
        <Modal
          onClose={handleCloseAddFavoritForBackgroundInformation}
          dense
          small
        >
          <ModalTitle>
              ...
              {equipeList.map((eq, index) => (
                <tr key={index}>
                  ...
                  <td style={{ padding: "5px", border: "1px solid #ccc" }}>
                    ...
                    >
                      {[...eq.superviseurs, ...eq.autreSuperviseurs].map((e) =>
                        e.displayName ? (
                          <span
                            key={e.displayName}
                            style={{
  ...
  const handleSelectFavoritForPerformance = (value) => {
    ...
    setSelectedElementForPerformances(
      currentFavorit?.indicators?.map((i) => i.indicator)
    );
              ...
              >
                <div>
                  <Button
                    ...
                    primary
                    icon={
                      <TbSelect style={{ fontSize: "18px", color: "#fff" }} />
                    }
                    onClick={() =>
                ...
                {selectedIndicators.length > 0 && (
                  <div style={{ marginLeft: "10px" }}>
                    <Popover
                      trigger="hover"
                      content={<>{translate("Enregistrer_Comme_Favorites")}</>}
                    >
                      <Button
                        disabled={selectedIndicators.length > 0 ? false : true}
                        small
  ...
  const handleInputSupervisors = (values, index) => {
    setInputFields(
      inputFields.map((field, fieldIndex) => {
        if (index === fieldIndex) {
          const supervisorsList = values.map((val) =>
  ...
  const handleCloseOrgUnitForm = (org, index) => {
    setSelectedOrganisationUnits(
      selectedOrganisationUnits.filter((ou) => ou.id !== org.id)
    );
    setInputFields(
      inputFields.filter((o) => o.organisationUnit?.id !== org.id)
    );
  };
            ...
            >
              ...
              <div style={{ padding: "10px" }}>
                <Row gutter={[10, 10]}>
                  ...
                  {selectedProgram?.fieldConfig?.supervisor?.dataElements
                    ?.length > 0 && (
                    <Col sm={24} md={24}>
                      <div>
                        <div style={{ marginBottom: "5px" }}>
                        ...
                        <Row gutter={[10, 10]}>
                          <Col md={19} sm={24}>
                            <Input
                              ...
                              onChange={(event) =>
                                handleInputOtherSupervisor(event, index)
                              }
                            />
                          </Col>
                          <Col flex="auto" style={{ textAlign: "right" }}>
                            <Button
                              primary
                              onClick={() =>
                                handleInputAddOtherSupervisors(index)
                              }
                            >
                              + {translate("Ajouter")}{" "}
                            </Button>
                              ...
                              >
                                ...
                                <div>
                                  <Popconfirm
                                    ...
                                    onConfirm={() =>
                                      handleDeleteOtherSupervisor(item, index)
                                    }
                                  >
                                    <RiDeleteBinLine
                                      style={{
  ...
  const handleCloseAgentForm = (ag) => {
    setSelectedAgents(
      selectedAgents.filter(
        (agent) => agent.trackedEntityInstance !== ag.trackedEntityInstance
      )
    );
    setInputFields(
      inputFields.filter((agent) => agent.trackedEntityInstance !== ag.id)
    );
  };
  ...
  const handleSelectOrganisationUnitGroupSet = (value) => {
    ...
    setSelectedOrganisationUnitGroupSet(
      organisationUnitGroupSets.find((org) => org.id === value)
    );
  };
  ...
  const handleSelectOrganisationUnitGroup = (value) => {
    ...
    setSelectedOrganisationUnitGroup(
      selectedOrganisationUnitGroupSet.organisationUnitGroups?.find(
        (org) => org.id === value
      )
    );
  };
  ...
  const handleSelectOrganisationUnitGroupForRandomCase = (values) => {
    setSelectedOrganisationUnitGroups(
      values.map((value) =>
        organisationUnitGroups?.find((org) => org.id === value)
      )
    );
  };
  ...
  const formatPeriod = (period, periodType) => {
    ...
    if (periodType === QUARTER)
      currentPeriod = `${dayjs(period).format("YYYY")}Q${dayjs(
        period
      ).quarter()}`;
  ...
  const handleDisplayIndicatorResult = async () => {
    try {
      ...
      const ouPayload = analyticResult.reduce((prev, cur) => {
        ...
        const currentIndicator = selectedIndicators.find(
          (i) => i.indicator?.id === currentElement
        );
        const score =
      ...
      if (analyticResult.length === 0) {
        setAnalyticErrorMessage(
          translate("Aucun_Donne_Trouver_Pour_Cette_Selection")
        );
      }
  ...
  const handleDisplayIndicatorResultForRandomCase = async () => {
    try {
      ...
      }&filter=organisationUnitGroups.id:in:[${selectedOrganisationUnitGroups
        .map((ou) => ou.id)
        .join(",")}]`;
      const response = await axios.get(route);
            ...
            <Col sm={24} md={6}>
              <div style={{ display: "flex", marginTop: "30px" }}>
                <Checkbox
                  ...
                  onChange={() =>
                    setInputMeilleurPositif(!inputMeilleurPositif)
                  }
                />
                <span
  ...
  const handleOkAnalyticComponentForPerformanceModal = () => {
    setSelectedIndicators(
      generateArrayOfFieldsInputForPerformances(selectedElementForPerformances)
    );
  ...
  const handleCancelAnalyticComponentForPerformanceModal = () => {
    setVisibleAnalyticComponentPerformanceModal(false);
    setSelectedElementForPerformances(
      selectedIndicators?.map((ind) => ind.indicator)
    );
  };
  ...
  const RenderAnalyticComponenPerformancetModal = () =>
    visibleAnalyticComponentPerformanceModal ? (
      <Modal
        onClose={() => handleCancelAnalyticComponentForPerformanceModal()}
        large
      >
        <ModalTitle>
          <div style={{ fontWeight: "bold", fontSize: "16px" }}>
  ...
  const handleChangeSelectionTypeConfigurationForBackgroundInformation = ({
    value,
  }) => {
    setSelectedProgramStage(null);
    setMappingConfigs([]);
  ...
  const handleSelectBackgroundInformationFavorit = (value) => {
    const currentFav = favoritBackgroundInformationList.find(
      (b) => b.id === value
    );
    if (currentFav) {
                ...
                <div>
                  ...
                  <Select
                    options={favoritBackgroundInformationList
                      .filter(
                        (f) => f.program?.id === selectedProgram?.program?.id
                      )
                      .map((favorit) => ({
                        label: favorit.name,
                        value: favorit.id,
                    ...
                    <div>
                      <div>
                        <Radio
                          ...
                          value={ELEMENT_GROUP}
                          checked={
                            selectedDataElementFromWhere === ELEMENT_GROUP
                          }
                        />
                      </div>
                      <div>
                      ...
                      <Row gutter={[10, 10]}>
                        {selectedProgramStage &&
                          selectedDataElementFromWhere === ELEMENT_GROUP && (
                            <Col md={24}>
                              <div style={{ marginBottom: "5px" }}>
                                {translate("Group_Element_Donnee")}
                              </div>
                              <Select
                                options={dataElementGroups.map((elGroup) => ({
                              ...
                              <Select
                                options={
                                  selectedProgramStage?.programStageDataElements
                                    ...
                                    ?.map((progStageDE) => ({
                                      label:
                                        progStageDE.dataElement?.displayName,
  ...
  const handleSelectCheckbox = (orgUnit) => {
    if (selectedOrganisationUnits.map((ou) => ou.id).includes(orgUnit.id)) {
      setSelectedOrganisationUnits(
        selectedOrganisationUnits.filter((ou) => ou.id !== orgUnit.id)
      );
      setInputFields(
        inputFields.filter((o) => o.organisationUnit?.id !== orgUnit.id)
      );
    } else {
      setSelectedOrganisationUnits([
        ...selectedOrganisationUnits,
    ...
    <tr style={{ backgroundColor, color: "#000" }} key={index}>
      <td style={{ border: "1px solid #ccc", padding: "5px" }}>
        <AntCheckbox
          ...
          checked={selectedOrganisationUnits
            .map((ou) => ou.id)
            .includes(an.orgUnit?.id)}
        />
      </td>
      <td
  ...
  const onOrgUnitSelected = (value) => {
    return (
      selectedPlanificationType === ORGANISATION_UNIT && loadTEIs(value.id)
    );
  };
  ...
  const handleSelectCheckboxAgent = (value) => {
    if (
      selectedAgents
        .map((ag) => ag.trackedEntityInstance)
        .includes(value.trackedEntityInstance)
    ) {
      setSelectedAgents(
        selectedAgents.filter(
          (ag) => ag.trackedEntityInstance !== value.trackedEntityInstance
        )
      );
      setInputFields(
        inputFields.filter(
          (o) => o.trackedEntityInstance !== value.trackedEntityInstance
        )
      );
    } else {
      setSelectedAgents([
        ...selectedAgents,
        teisList.find(
          (tei) => tei.trackedEntityInstance === value.trackedEntityInstance
        ),
      ]);
    }
  };
          ...
          if (currentEnrollment) {
            const payload = {
              ...
              agentName: `${
                ...
                current.attributes?.find(
                  (att) =>
                    att.attribute === selectedProgram?.attributeFirstName?.id
                )?.value || ""
              }`,
            };
            ...
            for (let ind of selectedIndicators) {
              ...
              const dValue =
                dValues.reduce((pv, cr) => cr.value + cr.value, 0) || 0;
              ...
              indicatorPayload[`${ind.indicator?.id}`] = {
                ...
                indicatorScore:
                  dValue * (ind.weight && ind.weight > 0 ? ind.weight : 1),
              };
            }
                          ...
                          return (
                            <>
                              <div>
                                <AntCheckbox
                                  onChange={() =>
                                    handleSelectCheckboxAgent(cell.getValue())
                                  }
                                  checked={selectedAgents
                                    .map((ag) => ag.trackedEntityInstance)
                                    .includes(
                                      cell.getValue()?.trackedEntityInstance
                                    )}
                                />
                              </div>
                            </>
                              ...
                              <div>
                                <AntCheckbox
                                  ...
                                  checked={selectedAgents
                                    .map((ag) => ag.trackedEntityInstance)
                                    .includes(
                                      row.original.trackedEntityInstance
                                    )}
                                />
                              </div>
                            </>
  ...
  const initAgentInput = (agentList) => {
    ...
    for (let ag of agentList) {
      if (
        inputFields
          .map((inp) => inp.trackedEntityInstance)
          .includes(ag.trackedEntityInstance)
      ) {
        newList.push(
          inputFields.find(
            (inp) => inp.trackedEntityInstance === ag.trackedEntityInstance
          )
        );
      } else {
