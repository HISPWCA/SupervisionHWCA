...
import ALL_STORE_FILE from '../datastores/userDataStorePayload/DEMO_3_STORE.json';
...
export const Body = () => {
    ...
    const updateBackgroundInformationsDataStore = async () => {
        try {
            ...
            const deCompletnessList = await loadDataStore(process.env.REACT_APP_DE_COMPLETNESS_KEY, null, null, []);
            const dsCompletnessList = await loadDataStore(process.env.REACT_APP_DS_COMPLETNESS_KEY, null, null, []);
                            ...
                            ? {
                                              ...
                                              (ind.selectedSourceConsistency &&
                                                  indicatorsList
                                                      ?.find(
                                                          group =>
                                                              group.name ===
                                                              favorite.formState.selectedGlobalProgramArea?.name
                                                      )
                                                      ?.children?.find(
                                                          child =>
                                                              child.name === ind.selectedSourceConsistency?.name ||
                                                              child.name_fr === ind.selectedSourceConsistency?.name
                                                      )) ||
                                              ind.selectedSourceConsistency
                                      })) || [],
                                  indicators:
                                          ...
                                          selectedSourceIndicator:
                                              (ind.selectedSourceIndicator &&
                                                  indicatorsList
                                                      ...
                                                      ?.children?.find(
                                                          child =>
                                                              child.name === ind.selectedSourceIndicator?.name ||
                                                              child.name_fr === ind.selectedSourceIndicator?.name
                                                      )) ||
                                              ind.selectedSourceIndicator
                                      })) || [],
                                          ...
                                          selectedSourcePrimary:
                                              (recoup.selectedSourcePrimary &&
                                                  crosschecksList
                                                      ...
                                                      ?.children?.find(
                                                          child =>
                                                              child.name === recoup.selectedSourcePrimary?.name ||
                                                              child.name_fr === recoup.selectedSourcePrimary?.name
                                                      )) ||
                                              recoup.selectedSourcePrimary,
                                          ...
                                          selectedSourceSecondary:
                                              (recoup.selectedSourceSecondary &&
                                                  crosschecksList
                                                      ...
                                                      ?.children?.find(
                                                          child =>
                                                              child.name === recoup.selectedSourceSecondary?.name ||
                                                              child.name_fr === recoup.selectedSourceSecondary?.name
                                                      )) ||
                                              recoup.selectedSourceSecondary
                                      })) || [],
                                  ...
                                  completeness: favorite.formState?.completeness
                                      ? {
                                            ...
                                            register: favorite.formState?.completeness?.register
                                                ? registersList
                                                      ...
                                                      ?.children?.find(
                                                          child =>
                                                              child.name ===
                                                                  favorite.formState?.completeness?.register?.name ||
                                                              child.name_fr ===
                                                                  favorite.formState?.completeness?.register?.name
                                            ...
                                            sourceDocuments:
                                                favorite.formState?.completeness?.sourceDocuments?.map(sourceDoc => ({
                                                    ...sourceDoc,
                                                    selectedSourceDS: sourceDoc.selectedSourceDS
                                                        ? dsCompletnessList
                                                              ?.find(
                                                                  group =>
                                                                      group.name ===
                                                              ...
                                                              ?.children?.find(
                                                                  child =>
                                                                      child.name === sourceDoc.selectedSourceDS?.name ||
                                                                      child.name_fr === sourceDoc.selectedSourceDS?.name
                                                              ) || sourceDoc.selectedSourceDS
                                                        : null || null
                                                })) || [],

                                            dataElements:
                                                favorite.formState?.completeness?.dataElements?.map(sourceDE => ({
                                                    ...sourceDE,
                                                    selectedSourceDE: sourceDE.selectedSourceDE
                                                        ? deCompletnessList
                                                              ?.find(
                                                                  group =>
                                                                      group.name ===
                                                              ...
                                                              ?.children?.find(
                                                                  child =>
                                                                      child.name === sourceDE.selectedSourceDE?.name ||
                                                                      child.name_fr === sourceDE.selectedSourceDE?.name
                                      ...
                                      ? {
                                            ...
                                            id:
                                                [
                                                    ...
                                                    ?.find(
                                                        ind =>
                                                            ind.name === config.indicator?.id ||
                                                            ind.name_fr === config.indicator?.id
                    ...
                    indicatorsMappingList?.map(indMapping => ({
                        ...
                        indicator: indMapping.indicator
                            ? indicatorsList
                                  ...
                                  ?.children?.find(
                                      child =>
                                          child.name === indMapping.indicator || child.name_fr === indMapping.indicator
        ...
        try {
            ...
            await Promise.all([
                ...
                loadDataStore(
                    ...
                    null,
                    null,
                    STORES[process.env.REACT_APP_SUPERVISIONS_CONFIG_KEY]
                ),
                loadDataStore(
                    process.env.REACT_APP_INDICATORS_CONFIG_KEY,
                    null,
                    null,
                    STORES[process.env.REACT_APP_INDICATORS_CONFIG_KEY]
                ),
                loadDataStore(
                    ...
                    STORES[process.env.REACT_APP_ANALYSES_CONFIG_KEY]
                ),
                loadDataStore(
                    process.env.REACT_APP_PERFORMANCE_FAVORITS_KEY,
                    null,
                    null,
                    STORES[process.env.REACT_APP_PERFORMANCE_FAVORITS_KEY]
                ),
                loadDataStore(
                    process.env.REACT_APP_BACKGROUND_INFORMATION_FAVORITS_KEY,
                    null,
                ...
                loadDataStore(
                    ...
                    null,
                    null,
                    STORES[process.env.REACT_APP_VISUALIZATION_KEY]
                ),
                loadDataStore(
                    process.env.REACT_APP_CROSS_CUT_KEY,
                    null,
                    null,
                    STORES[process.env.REACT_APP_CROSS_CUT_KEY]
                ),
                loadDataStore(
                    process.env.REACT_APP_INDICATORS_KEY,
                    null,
                    null,
                    STORES[process.env.REACT_APP_INDICATORS_KEY]
                ),
                loadDataStore(
                    process.env.REACT_APP_REGISTRES_KEY,
                    null,
                    null,
                    STORES[process.env.REACT_APP_REGISTRES_KEY]
                ),
                loadDataStore(
                    process.env.REACT_APP_DE_COMPLETNESS_KEY,
                    null,
                    null,
                    STORES[process.env.REACT_APP_DE_COMPLETNESS_KEY]
                ),
                loadDataStore(
                    process.env.REACT_APP_DS_COMPLETNESS_KEY,
                    null,
                    null,
                    STORES[process.env.REACT_APP_DS_COMPLETNESS_KEY]
                ),
                loadDataStore(
                    process.env.REACT_APP_INDICATORS_MAPPING_KEY,
                    null,
                    null,
                    STORES[process.env.REACT_APP_INDICATORS_MAPPING_KEY]
                ),
                loadDataStore(
                    process.env.REACT_APP_META_INFOS_NAME,
                    null,
                    null,
                    STORES[process.env.REACT_APP_META_INFOS_NAME]
                ),
                loadDataStore(
                    process.env.REACT_APP_PERIODS_CONFIG_KEY,
                    null,
                    null,
                    STORES[process.env.REACT_APP_PERIODS_CONFIG_KEY]
                ),
                loadDataStore(
                    ...
                    >
                        ...
                        <div style={{ marginLeft: '20px' }}>
                            ...
                            {updateMessages?.map(message => (
                                <div key={message} style={{ display: 'flex', gap: '5px', marginTop: '10px' }}>
                                    <span>{message}</span>
                                    <span>
