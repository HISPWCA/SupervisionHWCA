...
const Favorites = ({ me }) => {
    ...
    const [loadingDeleteFavoritBackgroundInformations, setLoadingDeleteFavoritBackgroundInformations] = useState(false);
    ...
    const [visibleAnalyticComponentModalForCrossCheck, setVisibleAnalyticComponentModalForCrossCheck] = useState(false);
                    ...
                    indicators:
                        currStage.indicators
                            ...
                            ?.map((ind, index) => ({
                                ...
                                selectedSourceMargin: existingFormState?.indicators[index]?.selectedSourceMargin || null
                            })) || [],
                    recoupements:
                        currStage.recoupements
                            ...
                            ?.map((ind, index) => ({
                                ...ind,
                                selectedSourceProgramArea:
                                    existingFormState?.recoupements[index]?.selectedSourceProgramArea || null,
                    ...
                    consistencyOvertimes:
                        currStage.consistencyOvertimes
                            ...
                            ?.map((ind, index) => ({
                                ...ind,
                                selectedSourceProgramArea:
                                    existingFormState?.consistencyOvertimes[index]?.selectedSourceProgramArea || null,
                                selectedSourceConsistency:
                                    existingFormState?.consistencyOvertimes[index]?.selectedSourceConsistency || null,
                                selectedSourceMargin:
                                    existingFormState?.consistencyOvertimes[index]?.selectedSourceMargin || null
                            })) || [],

                    completeness: {
                        ...currStage.completeness,

                        selectedNbrDocumentsSourceToShow: currStage.completeness?.selectedNbrDocumentsSourceToShow,
                            ...
                            (currStage.completeness?.programAreaDE &&
                                currStage.completeness?.dataElements
                                    ...
                                    ?.map((ind, index) => ({
                                        ...ind,
                                        selectedSourceDE:
                                            existingFormState?.completeness?.dataElements[index]?.selectedSourceDE ||
                                            null
                                    }))) ||
                            [],
                        sourceDocuments:
                            (currStage.completeness?.sourceDocuments &&
                                currStage.completeness?.sourceDocuments
                                    ...
                                    ?.map((ind, index) => ({
                                        ...ind,
                                        selectedSourceDS:
                                            existingFormState?.completeness?.sourceDocuments[index]?.selectedSourceDS ||
                                            null
                                    }))) ||
                            [],
                    ...
                    newRecoupementList.push({
                        ...
                        source:
                            initValuesList
                                ...
                                ?.recoupements?.find(iRec => iRec.position === rec?.position)?.source || null
                    });
                }
        ...
        setFormState({
            ...
            selectedProgramStage:
                sup.programStageConfigurations?.length === 0
                    ? programStages.find(pstage => pstage.id === sup.programStageConfigurations[0]?.programStage?.id)
                    : null
        });
    };
    ...
    const getEachDataElements = () => {
        ...
        for (let indicator of formState?.indicators?.slice(0, formState?.nbrIndicatorsToShow)) {
            ...
            if (payloadIndicator.dataElement?.id && payloadIndicator.indicator?.id) newList.push(payloadIndicator);
        ...
        for (let recoupement of formState?.recoupements?.slice(0, formState?.recoupements?.length - 1)) {
            ...
            if (payloadSecondary.dataElement?.id && payloadSecondary.indicator?.id) newList.push(payloadSecondary);
            if (payloadMargin.dataElement?.id && payloadMargin.indicator?.id) newList.push(payloadMargin);
        }
        ...
        if (formState?.completeness) {
            if (formState?.completeness?.register && formState?.completeness?.selectedRegister) {
                ...
                if (payloadRegistre.dataElement?.id && payloadRegistre.indicator?.id) newList.push(payloadRegistre);
            }
            ...
            if (
                ...
                if (payloadNbrDocumentsSourceToShow.dataElement?.id && payloadNbrDocumentsSourceToShow.indicator?.id)
    ...
    const getEachDataElementsForRDQA = () => {
        ...
        for (let indicator of indicatorFieldsForRDQA) {
            ...
            for (let recoupement of indicator?.recoupements) {
                ...
                if (recoupementPayload.dataElement && recoupementPayload.indicator) newList.push(recoupementPayload);
            }
        }
                            ...
                            <div style={{ marginTop: '10px' }}>
                                ...
                                <Input
                                    ...
                                    onChange={event =>
                                        setFormState({
                                            ...formState,
                                            inputFavorisNameForBackgroundInforation: event.target.value
                                        })
                                    }
                                />
                            </div>
    ...
    const handleDeleteFavoritesForBackgroundInformations = async () => {
        try {
            ...
            if (formState?.selectedBackgroundInformationFavorit) {
                ...
                await saveDataToDataStore(process.env.REACT_APP_BACKGROUND_INFORMATION_FAVORITS_KEY, newFavoritList);
                            ...
                            <div style={{ marginTop: '10px' }}>
                                <div>
                                    <Radio
                                        ...
                                        value={FAVORIS}
                                        checked={formState?.selectedBackgroundInformationTypeConfiguration === FAVORIS}
                                    />
                                </div>
                                <div>
                                    <Radio
                                        ...
                                        onChange={handleChangeSelectionTypeConfigurationForBackgroundInformation}
                                        value={DIRECTE}
                                        checked={formState?.selectedBackgroundInformationTypeConfiguration === DIRECTE}
                                    />
                                </div>
                            </div>
                        </Col>
                                ...
                                <div>
                                    ...
                                    <Select
                                        options={favoritBackgroundInformationList
                                            .filter(f => f.program?.id === formState?.selectedProgram?.program?.id)
                                            .map(favorit => ({
                                                label: favorit.name,
                                                value: favorit.id
                                            }))}
                                        ...
                                        disabled={loadingBackgroundInformationFavoritsConfigs}
                            ...
                            <div style={{ marginTop: '18px', display: 'flex' }}>
                                {formState?.selectedBackgroundInformationTypeConfiguration === FAVORIS && (
                                    <div style={{ marginRight: '10px' }}>
                                        <Popconfirm
                                            title={translate('Confirmation_Suppression_Configuration')}
                                            onConfirm={handleDeleteFavoritesForBackgroundInformations}
                                        >
                                            <Button
                                                destructive
                                                loading={loadingDeleteFavoritBackgroundInformations}
                                                icon={
                                                    <RiDeleteBinLine
                                                        style={{
                                                            color: 'white',
                                ...
                                <div>
                                    ...
                                    >
                                        {formState?.selectedBackgroundInformationTypeConfiguration === DIRECTE
                      ...
                      children: curr.children?.filter(d =>
                          dataStoreIndicatorsMapping?.find(mapping =>
                              d.isNotInDHIS2 === true
                                  ...
                                  : d.isStock === true
                                  ? true
                                  : mapping.group === curr.name && mapping.indicator === d.id && mapping.dhis2?.id
                          )
                      )
                  });
        ...
        <div>
            ...
            <div style={{ marginTop: '10px', padding: '10px' }}>
                <Row gutter={[12, 12]}>
                    ...
                    <Col sm={24} md={18}>
                        {formState?.selectedProgramStage && formState?.selectedProgram?.configurationType === DQR && (
                            <FavoriteGenerateIndicatorsFieldsDQR
                                formState={formState}
                                setFormState={setFormState}
                                dataStoreIndicators={filteredIndicatorsFromIndicatorsMapping() || []}
                        ...
                        {formState?.selectedProgramStage && formState?.selectedProgram?.configurationType === RDQA && (
                ...
                <ModalContent>
                    <div>
                        ...
                        <Select
                            ...
                            onChange={value =>
                                setSelectedCrosscheck(dataStoreCrosschecks?.find(cross => cross.name === value))
                            }
                            ...
                            <Select
                                ...
                                onChange={value =>
                                    setSelectedCrosscheckChild(
                                        selectedCrosscheck?.children?.find(cross => cross.name === value)
                                    )
                                }
                            ...
                            onClick={() => {
                                setFormIndicatorConfiguration({
                                    ...
                                    selectedRecoupements: formIndicatorConfiguration?.selectedRecoupements?.map(rec => {
                                        if (
                                            formIndicatorConfiguration?.currentElement?.dataElement?.id ===
                                            rec?.dataElement?.id
                                        ) {
                                            return {
                                                ...rec,
