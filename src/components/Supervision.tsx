...
const Supervision = ({ me }) => {
        ...
        return selectedSupervisionsConfigProgram?.planificationType === ORGANISATION_UNIT
            ? [
                  ...
                  },
                  {
                      ...
                      Cell: ({ cell, row }) =>
                          cell.getValue()?.trim()?.length > 0 && cell.getValue()?.trim()?.split(',')?.length > 0 ? (
                              <div
                                  style={{
                                      display: 'flex',
                                      alignItems: 'center',
                              ...
                              <>
                                  <span
                                      ...
                                      style={{
                                          ...
                                          background: getStatusNameAndColor(cell.getValue())?.color?.background,
                            ...
                            selectedSupervisionsConfigProgram?.fieldConfig?.supervisor?.dataElements?.reduce(
                                (prevEl, curr) => {
                                    const foundedDataValue = current.enrollments
                                        ?.filter(en => en.program === selectedSupervisionsConfigProgram?.program?.id)[0]
                            ...
                            ...prev,
                            {
                                ...
                                agent: `${
                                    current.attributes?.find(
                                        att => att.attribute === selectedSupervisionsConfigProgram?.attributeName?.id
                                    )?.value || ''
                                } ${
                                    current.attributes?.find(
                                        att =>
                                            att.attribute === selectedSupervisionsConfigProgram?.attributeFirstName?.id
                                    )?.value || ''
                                }`,
                            ...
                            ...enrollmentsList
                                ...
                                .map(en => ({
                                    ...
                                    agent: `${
                                        current.attributes?.find(
                                            att =>
                                                att.attribute === selectedSupervisionsConfigProgram?.attributeName?.id
                                        )?.value || ''
                                    } ${
                                        current.attributes?.find(
                                            att =>
                        ...
                        return [
                            ...
                            ...eventList
                                ...
                                .map(ev => ({
                                    ...
                                    superviseurs: selectedSupervisionsConfigProgram?.programStageConfigurations
    ...
    const generateTeiWithEnrollmentWithEvents = async payload => {
        try {
            ...
            if (payload.programStageConfig?.supervisorField?.length > 0) {
                ...
                let newSupervisorsList = [...payload.supervisors?.map(s => s.displayName), ...payload.otherSupervisors];
                ...
                newDataValueAsListofArrayForConsistencyOverTime = await Promise.all(
                    eventPayload.dataValues?.map(async dv => {
                        ...
                        if (foundOvertime) {
                            ...
                            if (periodType && foundAggrageMappingElement) {
                                newConsistencyOverTimePeriodTypes.push({
                                    position: foundOvertime.position,
                                    periodType
                                });
                            }

                            if (foundAggrageMappingElement) {
                                const elementCURRENT_PERIOD = foundOvertime?.currentPeriod;
                                const elementPERIOD_ONE_YEAR_AGO = foundOvertime?.currentPeriodOneYearAgo;
                                const elementLAST_PERIOD = foundOvertime?.lastPeriod;
                                const elementPERIOD1 = foundOvertime?.period1;
                                const elementPERIOD2 = foundOvertime?.period2;
                                const elementPERIOD3 = foundOvertime?.period3;

                                if (elementCURRENT_PERIOD) {
                                    const periodObject = getRightPeriodFormat(
                                        1,
                                        periodType,
                                        payload.periodVerification
                                    ...
                                    if (value) {
                                        newDvList.push({
                                            dataElement: elementCURRENT_PERIOD.id,
                                            value: value
                                        });
                                    }
                                }

                                if (elementPERIOD_ONE_YEAR_AGO) {
                                    const periodObject = getRightPeriodFormat(
                                        1,
                                        periodType,
                                        dayjs(payload.periodVerification).subtract(1, 'year')
                                    );
                                    ...
                                    if (value) {
                                        newDvList.push({
                                            dataElement: elementPERIOD_ONE_YEAR_AGO.id,
                                            value: value
                                        });
                                    }
                                }

                                if (elementLAST_PERIOD) {
                                    const periodObject = getRightPeriodFormat(
                                        1,
                                        periodType,
                                        payload.periodVerification
                                    ...
                                    if (value) {
                                        newDvList.push({
                                            dataElement: elementLAST_PERIOD.id,
                                            value: value
                                        });
                                    }
                                }
            ...
            const selectedConsistencyOverTimePeriodType =
                payload.programStageConfig?.selectedConsistencyOverTimePeriodType;
            ...
            const newDataValueList = newDataValueAsListofArrayForIndicators.reduce(
    ...
    const handleSelectIndicators = values =>
        setSelectedIndicators(values.map(val => dataStoreIndicatorConfigs.find(dsInd => dsInd.indicator?.id === val)));

    const handleSelectOrgUnitGroup = values =>
        setSelectedOrganisationUnitGroups(values.map(val => organisationUnitGroups.find(orgGp => orgGp.id === val)));
                ...
                <Card size="small" className="my-shadow">
                    {loadingAllSupervisionsFromTracker && (
                        <div style={{ display: 'flex', alignItems: 'center' }}>
                            ...
                            <span style={{ marginLeft: '20px' }}>{translate('Chargement_Des_Supervisions')}</span>
                        </div>
                    )}
                        ...
                        {isEditionMode && inputFields.length > 0 && (
                            <div style={{ marginTop: '15px' }}>
                                <Button
                                    ...
                                    icon={<ImCancelCircle style={{ color: '#fff', fontSize: '18px' }} />}
                                    destructive
                                >
                                    {translate('Annuler')}
                                </Button>
                        ...
                        <tbody>
                            {equipeList.length === 0 && (
                                <tr style={{ color: '#00000080' }}>
                                    <td colSpan={3} tyle={{ padding: '5px', border: '1px solid #ccc' }}>
                                        {translate('List_Vide')}
                                    </td>
                                </tr>
                            )}
                ...
                <Card bodyStyle={{ padding: '0px' }} className="my-shadow" size="small">
                    ...
                    <div style={{ padding: '10px' }}>
                        ...
                        {selectedSelectionTypeForPerformance === FAVORIS && (
                            <>
                                <div>
                                    <div style={{ marginBottom: '5px' }}>{translate('Selectionnez_La_Favorit')}</div>
                                    <Select
                            ...
                            >
                                <div>
                                    <Button
                                        small
                                        primary
                                        icon={<TbSelect style={{ fontSize: '18px', color: '#fff' }} />}
                                        onClick={() => setVisibleAnalyticComponentPerformanceModal(true)}
                                    >
                                                ...
                                                >
                                                    <div>
                                                        <InputNumber
                                                            value={ind.weight}
                                                            onChange={event =>
                                                                handleChangePoidsForFieldsPerformances(event, index)
                                                            }
                                                            style={{ width: '100%' }}
                                    ...
                                    <Col sm={24} md={24}>
                                        <div>
                                            <div style={{ marginBottom: '5px' }}>
                                                {
                                                    <span>
                                                        {translate('Periode')} (<span style={{ color: RED }}> * </span>)
                                                    </span>
                                                }
                                            </div>
                                            <DatePicker
                                        ...
                                        <Col sm={24} md={24}>
                                            <div>
                                                ...
                                                >
                                                    ...
                                                    <Popover
                                                        content={
                                                            ...
                                                            >
                                                                {translate('Recent_Verification_Period_Help')}
                                                            </div>
                                                        }
                                                    >
                                                ...
                                                <DatePicker
                                                    ...
                                                    onChange={period => handleInputPeriodVerification(period, index)}
                                                />
                                            </div>
                                        </Col>
                                    )}
        ...
        try {
            ...
            const ouPayload = analyticResult.reduce((prev, cur) => {
                ...
                if (prev[currentOrgUnit]) {
                    ...
                    prev[currentOrgUnit] = {
                        ...prev[currentOrgUnit],
                        scoreTotal: parseFloat(parseFloat(prev[currentOrgUnit].scoreTotal) + parseFloat(score)).toFixed(
                            1
                        )
                    };
                } else {
    ...
    const handleDisplayIndicatorResultForRandomCase = async () => {
        try {
            ...
            if (selectedOrganisationUnitGroups.length === 0) throw new Error(translate('OrgUnitGroup_Obligatoire'));
                        ...
                        {selectedOrganisationUnitGroupSet && (
                            <Col sm={24} md={6}>
                                <div style={{ marginTop: '20px' }}>
                                    ...
                                    <Select
                                        style={{ width: '100%' }}
                                        options={selectedOrganisationUnitGroupSet?.organisationUnitGroups?.map(org => ({
                                            value: org.id,
                                            label: org.displayName
                                        }))}
    ...
    const handleSearchByPerformances = async () => {
        try {
            ...
            if (!selectedOrganisationUnitSingle) throw new Error(translate('Veuillez_Selectionner_Unite_Organisation'));
                ...
                nouveauTeiList = teis.reduce((prev, current) => {
                    ...
                    if (currentEnrollment) {
                        const payload = {
                            ...current,
                            agentName: `${
                                current.attributes?.find(att => att.attribute === selectedProgram?.attributeName?.id)
                                    ?.value || ''
                            } ${
                                current.attributes?.find(
                            ...
                            const dValues =
                                dataValues.filter(
                                    dv =>
                                        ...
                                        dayjs(selectedPeriod).format('YYYYMM') === dayjs(dv.period).format('YYYYMM')
                                ) || [];
                                                        ...
                                                        <div>
                                                            <AntCheckbox
                                                                ...
                                                                checked={selectedAgents
                                                                    ...
                                                                    .includes(cell.getValue()?.trackedEntityInstance)}
                                                            />
                                                        </div>
                                                    </>
                                                );
                                                    ...
                                                    <>
                                                        <div>
                                                            <AntCheckbox
                                                                ...
                                                                checked={selectedAgents
                                                                    .map(ag => ag.trackedEntityInstance)
                                                                    .includes(row.original.trackedEntityInstance)}
                                                            />
                                                        </div>
                                                    </>
                                                );
                ...
                <>
                    <Row gutter={[10, 10]}>
                        ...
                        {selectedSupervisionType === TYPE_SUPERVISION_ORGANISATION_UNIT &&
                            selectedPlanificationType === INDICATOR &&
                            selectedIndicators.length > 0 &&
                            selectedOrganisationUnits.length > 0 && <Col md={24}>{RenderOrganisationUnitForm(8)}</Col>}
                        {selectedSupervisionType === TYPE_SUPERVISION_ORGANISATION_UNIT &&
                            selectedPlanificationType === RANDOM &&
                            selectedOrganisationUnits.length > 0 && <Col md={24}>{RenderOrganisationUnitForm(8)}</Col>}
                    </Row>
