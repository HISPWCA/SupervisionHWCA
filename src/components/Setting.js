...
import {
    ...
    PAGE_CROSS_CHECK_MANAGAEMENT,
    PAGE_DATA_ELEMENT_MANAGEMENT,
...
const Setting = () => {
    ...
    const initFields = (fieldList = null) => {
        ...
        for (let i = 1; i <= +dqrConfig.nbrIndicator; i++) {
            newIndicators.push({
                ...
                DHIS2MonthlyValue1: fieldList?.indicators?.find(ind => ind.position === i)?.DHIS2MonthlyValue1 || null,
                DHIS2MonthlyValue2: fieldList?.indicators?.find(ind => ind.position === i)?.DHIS2MonthlyValue2 || null,
                DHIS2MonthlyValue3: fieldList?.indicators?.find(ind => ind.position === i)?.DHIS2MonthlyValue3 || null,
                DHIS2MonthlyValue4: fieldList?.indicators?.find(ind => ind.position === i)?.DHIS2MonthlyValue4 || null,
                DHIS2MonthlyValue5: fieldList?.indicators?.find(ind => ind.position === i)?.DHIS2MonthlyValue5 || null,
                DHIS2MonthlyValue6: fieldList?.indicators?.find(ind => ind.position === i)?.DHIS2MonthlyValue6 || null,
        ...
        for (let i = 1; i <= +dqrConfig.nbrRecoupement; i++) {
            newRecoupements.push({
                ...
                distributedStock: fieldList?.recoupements?.find(rec => rec.position === i)?.distributedStock || null,
                ...
                secondaryValue: fieldList?.recoupements?.find(rec => rec.position === i)?.secondaryValue || null,
        ...
        for (let i = 1; i <= +dqrConfig.nbrSourceDocumentCompleteness; i++) {
            newSourceDocumentCompleteness.push({
                ...
                keyWords: fieldList?.completeness?.sourceDocuments?.find(el => el.position === i)?.keyWords || []
            });
        }
    ...
    const loadVisualizations = async () => {
        try {
            const response = await axios.get(`${VISUALIZATIONS_ROUTE}?pageSize=100000&fields=id,displayName,name,type`);
    ...
    const handleDeleteVisatualizationProgramConfig = async item => {
        try {
            if (item) {
                const newList = dataStoreVisualizations.filter(dataFav => dataFav.program?.id !== item.program?.id);
            ...
            if (formState?.selectedConfigurationType === RDQA) {
                newProgramStageConfigurations = existingConfig
                    ? formState?.isFieldEditingMode && currentProgramstageConfigurationForRDQA
                        ? programStageConfigurations.map(p => {
                              if (p.programStage?.id === currentProgramstageConfigurationForRDQA?.programStage?.id) {
                                  return {
                                      ...p,
                                      ...
                                      statusSupervisionField: formStateForRDQA?.selectedStatusSupervisionDataElement,
                                      indicatorsFieldsConfigs:
                                          indicatorsFieldsConfigsForRDQA
                                              ?.filter(ind => ind.value)
                                              .map(ind => ({
                                                  ...ind,
                                                  recoupements: ind.recoupements?.filter(recoup => recoup.value) || []
                                              })) || []
                                  };
                              }
    ...
    const handleSaveIndicatorConfig = async () => {
        try {
            ...
            if (indicatorName && indicatorEtiquette) {
                ...
                setNotification({
                    show: true,
                    message: !currentItem ? translate('Configuration_Ajoutee') : translate('Mise_A_Jour_Succes'),
                    type: NOTIFICATION_SUCCESS
                });
                setSelectedIndicator(null);
                setIndicatorName(null);
                ...
                <div style={{ marginTop: '20px' }}>
                    ...
                    <div style={{ marginTop: '5px' }}>
                        <Radio
                            ...
                            checked={formState?.selectedSupervisionGenerationType === TYPE_GENERATION_AS_EVENT}
                        />
                    </div>
                </div>
                        ...
                        <div style={{ marginTop: '10px' }}>
                            <div>
                                <Radio
                                    ...
                                    checked={formState?.selectedPlanificationType === ORGANISATION_UNIT}
                                />
                            </div>
                            <div>
                                <Radio
            ...
            if (
                ...
                setFormState({
                    ...
                    completeness: {
                        ...
                        selectedNbrDocumentsSourceToShow: value?.completeness?.selectedNbrDocumentsSourceToShow,
                                ...
                                <Col md={12}>
                                    <div>
                                        ...
                                        <Select
                                            ...
                                            value={formState?.selectedSupervisorDataElements?.map(s => s.id)}
                                        />
                                    </div>
                                </Col>
                            )}
                                ...
                                <Col md={12}>
                                    <div>
                                        ...
                                        <Select
                                            ...
                                            value={formState?.selectedStatusSupervisionDataElement?.id}
                                            optionFilterProp="label"
                                            showSearch
                                            allowClear
                                        />
                                    ...
                                    <table style={{ borderCollapse: 'collapse', width: '100%' }}>
                                        ...
                                        <tbody>
                                            <tr>
                                                ...
                                                >
                                                    {translate('System_Auto_Generate_Attribute_ID')}
                                                </td>
                                                <td
                                                    style={{
                                                        border: '1px solid #00000070',
                                            ...
                                            {formState?.selectedConfigurationType !== NORMAL_PROGRAM && (
                                                <tr>
                                                    ...
                                                    >
                                                        {translate('Indicators_Period_Type')}
                                                    </td>
                                                    <td
                                                        style={{
                                                            border: '1px solid #00000070',
                                                ...
                                                <tr>
                                                    ...
                                                    >
                                                        <Select
                                                            ...
                                                            value={formState?.completeness.selectedRegister?.id}
                                                            optionFilterProp="label"
                                                            showSearch
                                                            allowClear
                                                        />
                                            ...
                                            {formState?.selectedConfigurationType !== NORMAL_PROGRAM && (
                                                <tr>
                                                    ...
                                                    >
                                                        {translate('Keys_Word_Global_Program_Area')}
                                                    </td>
                                                    <td
                                                        style={{
                                                            border: '1px solid #00000070',
                                                    ...
                                                    >
                                                        <TagsInput
                                                            ...
                                                            onChange={word =>
                                                                setFormState({
                                                                    ...formState,
                                                                    globalProgramAreaKeyWords: word || []
                                                                })
                                                            }
                                                        />
                                                    </td>
                                            ...
                                            {formState?.selectedConfigurationType !== NORMAL_PROGRAM && (
                                                <tr>
                                                    ...
                                                    >
                                                        <Select
                                                            placeholder={translate('Number_Of_DHIS2_Period')}
                                                            style={{
                                                                width: '307px'
                                                            }}
                                                            ...
                                                            showSearch
                                                            optionFilterProp="label"
                                                            value={formState?.indicators[0]?.viewMonthlyValue}
                                                            onChange={value => {
                                                                setFormState({
                                                                    ...formState,
                                                                    indicators:
                                    ...
                                    >
                                        <div style={{ marginRight: '10px' }}>
                                            <FiEdit
                                                ...
                                                onClick={() => handleEditProgramStageConfigurations(value)}
                                            />
                                        </div>
                                    </div>
                                )
                ...
                <Card className="my-shadow" size="small">
                    <div>
                        <div style={{ margin: '20px 0px' }}>
                            <>
                                <GenerateIndicatorsFieldsDQR formState={formState} setFormState={setFormState} />
                            </>
                        </div>
                    </div>
                </Card>
                ...
                <Card className="my-shadow" size="small">
                    <div>
                        <div style={{ fontWeight: 'bold', display: 'flex', alignItems: 'center' }}>
                            <span>{translate('Program_Stage_Configuration_Fields_For_Recoupement_And_Indicator')}</span>
                            <span
                                style={{
                                    background: 'orange',
                                    padding: '2px 5px',
                ...
                <div>
                    ...
                    <div style={{ margin: '10px 0px' }}>
                        <Row gutter={[10, 10]}>
                            ...
                            <Col md={12} sm={24}>
                                <div>
                                    <div style={{ marginBottom: '5px' }}>{translate('Groupe_Unite_Organisation')}</div>
                                    <Select
                                        options={organisationUnitGroups?.map(organisationUnitGroup => ({
                                        ...
                                        onChange={handleSelectOrganisationUnitGroupProgramStageForRDQA}
                                        showSearch
                                        allowClear
                        ...
                        <>
                            ...
                            >
                                    ...
                                    <div style={{ marginTop: '20px' }}>
                                        <div>
                                            <Radio
                                                ...
                                                value="VISUALIZATION"
                                                checked={selectedTypeForVisualization === 'VISUALIZATION'}
                                            />
                                        </div>
                                        <div style={{ marginTop: '5px' }}>
                                            <Radio
                                ...
                                {(selectedMaps.length > 0 || selectedVisualizations?.length > 0) && (
                                    <div style={{ marginTop: '10px' }}>
                                        <Button primary onClick={handleAddVisualizationToFavorisList}>
                                            +{translate('Ajouter')}
                                        </Button>
                                    </div>
                                )}
                            ...
                            <>
                                                ...
                                                render: value => (
                                                    <div>
                                                        <Popconfirm
                                                            ...
                                                            onConfirm={() => handleDeleteVisualizationConfig(value)}
                                                        >
                                                            <div>
                                                                <RiDeleteBinLine
                                                                    style={{
                                ...
                                >
                                    ...
                                    <div style={{ marginLeft: '10px' }}>
                                        ...
                                        >
                                            {formState?.isFieldEditingMode && <span>{translate('Mise_A_Jour')}</span>}
                                            {!formState?.isFieldEditingMode && <span>{translate('Enregistrer')}</span>}
                                        </Button>
                                    </div>
                                </div>
                            </>
                                            ...
                                            >
                                                <div style={{ marginRight: '10px' }}>
                                                    <FiEdit
                                                        ...
                                                        onClick={() => handleEditVisualizations(value)}
                                                    />
                                                </div>
                                                <Popconfirm
                                                    title={translate('Suppression_Configuration')}
                                                    description={translate('Confirmation_Suppression_Configuration')}
                                                    icon={<QuestionCircleOutlined style={{ color: 'red' }} />}
                                                    onConfirm={() => handleDeleteVisatualizationProgramConfig(value)}
                                                >
                                                    <div>
                                                        <RiDeleteBinLine
                                                            style={{
                                            ...
                                            >
                                                <div style={{ marginRight: '10px' }}>
                                                    <FiEdit
                                                        ...
                                                        onClick={() => handleEditPaymentConfig(value)}
                                                    />
                                                </div>
                                                <Popconfirm
                                                    title={translate('Suppression_Configuration')}
                                ...
                                <Row gutter={[8, 8]}>
                                    <Col md={12} sm={24}>
                                        <div>
                                            ...
                                            <Select
                                                options={indicatorGroups?.map(indicateurGroup => ({
                                    ...
                                    {selectedIndicatorGroup && (
                                        <Col md={12} sm={24}>
                                            <div>
                                                ...
                                                <Select
                                                    options={selectedIndicatorGroup.indicators?.map(progInd => ({
                                    ...
                                    {selectedProgram && (
                                        <Col md={12} sm={24}>
                                            <div>
                                                <div style={{ marginBottom: '5px' }}>{translate('Indicateurs')}</div>
                                                <Select
                                                    options={selectedProgram.programIndicators?.map(progInd => ({
                                                        label: progInd.displayName,
                                                        value: progInd.id
                                                    }))}
                                                    placeholder={translate('Indicateurs')}
                                            ...
                                            <>
                                                ...
                                                >
                                                    <FiEdit
                                                        ...
                                                        onClick={() => handleUpdateBtnClicked(value)}
                                                    />
                                                    <Popconfirm
                                                        title={translate('Suppression_Configuration')}
                                                        description={translate(
    ...
    const RenderPageIndicatorsMapping = () => <SettingIndicatorsMapping />;
    const RenderPageRegistersManagement = () => <SettingRegistersManagement />;
    const RenderPageCrossChecksManagement = () => <SettingCrossChecksManagement />;
    const RenderPageDataElementsCompletenessManagement = () => <SettingDataElementCompletenessManagement />;
    const RenderPageSourceDocumentsCompletenessManagement = () => <SettingSourceDocumentCompletenessManagement />;
        ...
        <div>
            <Row gutter={[8, 8]}>
                ...
                <Col md={20} sm={24}>
                    ...
                    {selectedTypeSupervisionPage === PAGE_CONFIG_VISUALIZATION && RenderPageVisualizationsConfig()}
