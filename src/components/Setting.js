...
const Setting = () => {
  ...
  const [selectedIndicatorType, setSelectedIndicatorType] =
    useState(PROGRAM_INDICATOR);
  const [selectedTEIProgram, setSelectedTEIProgram] = useState(null);
  const [
    selectedSupervisionGenerationType,
    setSelectedSupervisionGenerationType,
  ] = useState(TYPE_GENERATION_AS_TEI);
  const [selectedProgram, setSelectedProgram] = useState(null);
  const [selectedTypeSupervisionPage, setSelectedTypeSupervisionPage] =
    useState(PAGE_CONFIG_SUPERVISION);
  ...
  const [selectedAttributesToDisplay, setSelectedAttributesToDisplay] =
    useState([]);
  const [
  ...
  const [selectedPlanificationType, setSelectedPlanificationType] =
    useState(ORGANISATION_UNIT);
  const [selectedAttributeNameForAgent, setSelectedAttributeNameForAgent] =
    useState(null);
  const [
    selectedAttributeFirstNameForAgent,
    setSelectedAttributeFirstNameForAgent,
  ] = useState(null);

  const [inputLibellePayment, setInputLibellePayment] = useState("");
  const [inputMontantConstantPayment, setInputMontantConstantPayment] =
    useState(0);
  const [inputFraisMobileMoneyPayment, setInputFraisMobileMoneyPayment] =
    useState(0);

  const [loadingPrograms, setLoadingPrograms] = useState(false);
  const [loadingIndicatorGroups, setLoadingIndicatorGroups] = useState(false);
  const [loadingSaveSupervionsConfig, setLoadingSaveSupervionsConfig] =
    useState(false);
  const [loadingSaveIndicatorsConfig, setLoadingSaveIndicatorsConfig] =
    useState(false);
  const [loadingIndicators, setLoadingIndicators] = useState(false);
  const [loadingDataElements, setLoadingDataElements] = useState(false);
  const [loadingAddAnalyseConfigs, setLoadingAddAnalyseConfigs] =
    useState(false);
  ...
  const handleSelectIndicatorGroupIND = (value) => {
    setSelectedIndicator(null);
    setSelectedIndicatorGroup(
      indicatorGroups.find((indGroup) => indGroup.id === value)
    );
  };
  ...
  const handleSelectIndicatorIND = (value) => {
    ...
    if (selectedIndicatorType === AGGREGATE_INDICATOR)
      currentIndicator = selectedIndicatorGroup.indicators?.find(
        (ind) => ind.id === value
      );

    if (selectedIndicatorType === PROGRAM_INDICATOR)
      currentIndicator = selectedProgram.programIndicators?.find(
        (progInd) => progInd.id === value
      );
    ...
    try {
      if (item) {
        const existingDXMappingOfPrograms =
          mappingConfigs.filter(
            (mConfig) => mConfig?.program?.id === item?.program?.id
          ) || [];

        if (existingDXMappingOfPrograms.length > 0)
          throw new Error(translate("Configuration_Deja_Mapper"));

        const newList = mappingConfigSupervisions.filter(
          (mapConf) => mapConf.id !== item.id
        );
        await saveDataToDataStore(
          process.env.REACT_APP_SUPERVISIONS_CONFIG_KEY,
          newList,
  ...
  const handleDeleteAnalyseConfig = async (value) => {
    try {
      if (value) {
        const newList = analyseConfigs.filter(
          (analyseConf) => analyseConf.id !== value.id
        );
        await saveDataToDataStore(
          process.env.REACT_APP_ANALYSES_CONFIG_KEY,
          newList,
        ...
        <div style={{ marginTop: "20px" }}>
          <div style={{ marginTop: "5px" }}>
            <Radio
              ...
              checked={
                selectedSupervisionGenerationType === TYPE_GENERATION_AS_TEI
              }
            />
          </div>
          <div style={{ marginTop: "5px" }}>
    ...
    try {
      ...
      setSelectedProgramStage(
        programStageList.find(
          (psg) => psg.id === prog.fieldConfig?.supervisor?.programStage.id
        )
      );
      setSelectedDataElements(
      ...
      setSelectedStatutSupervisionDataElement(
        prog?.statusSupervision?.dataElement
      );
      setSelectedStatutPaymentProgramStage(
        programStageList.find(
          (psg) => psg.id === prog.statusPayment?.programStage?.id
        )
      );
  ...
  const RenderAttributesToDisplay = () => (
    <div style={{ marginTop: "20px" }}>
      <Card className="my-shadow" size="small">
        <div>
          <div style={{ fontWeight: "bold" }}>
            {translate("Configuration_Des_Attributes")}
          </div>
  ...
  const RenderProgramStageConfiguration = () => (
    <div style={{ marginTop: "20px" }}>
      <Card className="my-shadow" size="small">
        <div>
          <div style={{ fontWeight: "bold" }}>Program stage configuration</div>
          <div
  ...
  const handleAddPaymentConfig = () => {
    try {
      ...
      setNotification({
        show: true,
        message: isEditModePayment
          ? translate("Paiement_Mise_A_Jour")
          : translate("Paiement_Ajouter"),
        type: NOTIFICATION_SUCCESS,
      });
    } catch (err) {
      setNotification({
  ...
  const RenderStatusPaymentDataElementToUse = () => (
    <div style={{ marginTop: "20px" }}>
      <Card className="my-shadow" size="small">
        <div>
          <div style={{ fontWeight: "bold" }}>
            {translate("Configuration_Des_Paiements")}
          </div>
          <div>
            <Row gutter={[10, 10]}>
              <Col md={24}>
            ...
            >
              {isFieldEditingMode && (
                <div>
                  <Button
                    icon={
                      <CgCloseO style={{ color: "#fff", fontSize: "18px" }} />
                    }
                    destructive
                    onClick={() => {
                      setFieldEditingMode(false);
  ...
  const RenderUserAuthorizationContent = () => (
    <div>User authorizations content</div>
  );
  ...
  const RenderTypeSupervisionContent = () => (
    <div>
      <Row gutter={[8, 8]}>
        ...
        <Col md={20} sm={24}>
          {selectedTypeSupervisionPage === PAGE_CONFIG_INDICATORS &&
            RenderPageIndicatorConfig()}
          {selectedTypeSupervisionPage === PAGE_CONFIG_SUPERVISION &&
            RenderPageSupervisionConfig()}
          {selectedTypeSupervisionPage === PAGE_CONFIG_ANALYSE &&
            RenderPageAnalyseConfig()}
        </Col>
      </Row>
    </div>
  );
    ...
    <>
      ...
      >
        <TabBar>
          ...
          {0 > 1 && (
            <Tab
              selected={renderPage === PAGE_CONFIGURATION_USER_AUTHORIZATIONS}
              onClick={(_) =>
                setRenderPage(PAGE_CONFIGURATION_USER_AUTHORIZATIONS)
              }
            >
        ...
        <div style={{ padding: "20px", marginTop: "20px" }}>
          {renderPage === PAGE_CONFIGURATION_TYPE_SUPERVISIONS &&
            RenderTypeSupervisionContent()}
  ...
  useEffect(() => {
    loadPrograms();
    selectedTypeSupervisionPage === PAGE_CONFIG_INDICATORS &&
      initIndicatorConfigStates();
    selectedTypeSupervisionPage === PAGE_CONFIG_SUPERVISION &&
      initSupConfigStates();
    selectedTypeSupervisionPage === PAGE_CONFIG_ANALYSE &&
      initAnalyseConfigStates();
  }, []);
