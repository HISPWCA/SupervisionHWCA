...
const SettingAddFormModal = ({ getUpdatedElementsList, open, setOpen, dataStoreElements, elementName , dataStoreKey }) => {
    ...
    const handleDeleteElement = id => {
        ...
        if (id) {
            setNewIndicatorList(newElementsList.filter(i => id !== i.id));
            setCurrentItem(null);
        }
    };
    ...
    const handleSaveInfos = async () => {
        try {
            setLoadingSave(true);
            const lastDataStoreList = (await getUpdatedElementsList()) || [];
