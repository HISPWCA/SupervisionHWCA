...
export const loadDataStoreDEBUG = async (key_string, setLoading, setState, payload = null) => {
      try {
            if (!key_string) throw new Error('Veuillez préciser le key_string du datastore à récupérer');

            setLoading && setLoading(true);

            const route = `${DATA_STORE_ROUTE}/${process.env.REACT_APP_DATA_STORE_NAME_DEBUG}/${key_string}`;
            const response = await axios.get(route);
            const data = response.data;
            ...
            setLoading && setLoading(false);
            createDataToDataStoreDEBUG(key_string, payload ? payload : []);
            console.clear();
      }
};

export const saveDataToDataStoreDEBUG = async (key_string, payload, setLoading, setState, setErrorMessage) => {
      try {
            if (!key_string) throw new Error('Veuillez préciser le key_string du datastore à récupérer');
...
export const createDataToDataStoreDEBUG = async (key_string, payload) => {
      try {
            if (!key_string) throw new Error('Veuillez préciser le key_string du datastore à récupérer');
